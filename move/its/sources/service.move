...
public fun deploy_remote_interchain_token<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"deploy_remote_interchain_token".to_ascii_string());
...
public fun receive_interchain_transfer<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"receive_interchain_transfer".to_ascii_string());
...
public fun receive_interchain_transfer_with_data<T>(
    ...
    self
        ...
        .check(
            VERSION,
            b"receive_interchain_transfer_with_data".to_ascii_string(),
        );
...
public fun receive_deploy_interchain_token<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"receive_deploy_interchain_token".to_ascii_string());
    let (_, payload) = decode_approved_message(self, approved_message);
    let mut reader = abi::new_reader(payload);
    assert!(
...
public fun give_unregistered_coin<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"give_unregistered_coin".to_ascii_string());
...
public fun mint_as_distributor<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"mint_as_distributor".to_ascii_string());
...
public fun mint_to_as_distributor<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"mint_to_as_distributor".to_ascii_string());
...
public fun burn_as_distributor<T>(
    ...
    self
        .version_control()
        .check(VERSION, b"burn_as_distributor".to_ascii_string());
    let coin_management = self.coin_management_mut<T>(token_id);
    let distributor = channel.to_address();
...
public fun set_trusted_addresses(
    ...
    self
        .version_control()
        .check(VERSION, b"set_trusted_addresses".to_ascii_string());
    let (
        source_chain,
        _,
        source_address,
...
fun decode_approved_message(
    ...
    if (reader.read_u256() == MESSAGE_TYPE_RECEIVE_FROM_HUB) {
        ...
        assert!(
            self.get_trusted_address(source_chain).into_bytes() == ITS_HUB_ROUTING_IDENTIFIER,
            EUntrustedChain,
        );
    } else {
        assert!(
...
fun test_deploy_remote_interchain_token() {
    ...
    assert!(
        message_ticket.destination_address() == its.get_trusted_address(destination_chain),
    );
...
fun test_receive_interchain_transfer_with_data() {
    ...
    let (
        ...
        received_data,
        received_coin,
    ) = receive_interchain_transfer_with_data<COIN>(
        &mut its,
        approved_message,
        &channel,
        &clock,
