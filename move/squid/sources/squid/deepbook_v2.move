...
fun get_max_quote_from_base<T1, T2>(
    pool: &Pool<T1, T2>,
    price: u64,
    depth: u64,
    max_base: u64,
): (u64, u64) {
    let filled_base_quantity = if (max_base >= depth) { depth } else {
        max_base
    };
    // If a bit is rounded down, the pool will take this as a fee.
    let (_, mut filled_quote_quantity) = unsafe_mul_round(
...
fun get_max_base_from_quote<T1, T2>(
    pool: &Pool<T1, T2>,
    price: u64,
    depth: u64,
    max_quote: u64,
    ...
    let maker_quote_quantity = maker_quote_quantity_without_commission +
    taker_commission;
    ...
    if (max_quote > maker_quote_quantity) {
        ...
        (_, filled_quote_quantity_without_commission) =
            unsafe_div_round(
                max_quote,
        ...
        filled_quote_quantity =
            filled_quote_quantity_without_commission + taker_commission;
    };

    (filled_quote_quantity, filled_base_quantity)
}

fun predict_base_for_quote<T1, T2>(
    pool: &Pool<T1, T2>,
    amount: u64,
    lot_size: u64,
    clock: &Clock,
    ...
    let (prices, depths) = clob::get_level2_book_status_bid_side(
        pool,
        0,
        max_price,
        clock,
...
fun predict_quote_for_base<T1, T2>(
    pool: &Pool<T1, T2>,
    amount: u64,
    lot_size: u64,
    clock: &Clock,
    ...
    let max_price = (1u128 << 64 - 1 as u64);
    let (prices, depths) = clob::get_level2_book_status_ask_side(
        pool,
        0,
        max_price,
        clock,
...
public fun estimate<T1, T2>(
    ...
    assert!(
        &bcs.peel_vec_u8() == &type_name::get < T1 > ()
            .into_string()
            .into_bytes(),
        EWrongCoinType,
    );
    ...
    assert!(
        &bcs.peel_vec_u8() == &type_name::get < T2 > ()
            .into_string()
            .into_bytes(),
        EWrongCoinType,
    );
    ...
    if (swap_data.has_base) {
        ...
        if (leftover > 0) {
            if (swap_data.should_sweep) {
                squid
                    .coin_bag()
                    .store_balance<T1>(base_balance.split(leftover));
            } else {
                self.coin_bag().store_balance<T1>(base_balance.split(leftover));
            };
        };
