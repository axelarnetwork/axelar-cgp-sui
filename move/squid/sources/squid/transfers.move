module squid::transfers {
    ...
    use interchain_token_service::{interchain_token_service::{Self, InterchainTokenService}, token_id::{Self, TokenId}};
    ...
    use sui::{bcs::BCS, clock::Clock, coin};
    ...
    public(package) fun sui_estimate_move_call(package_id: address, mut bcs: BCS, swap_info_arg: vector<u8>): MoveCall {
    ...
    public(package) fun its_estimate_move_call(package_id: address, mut bcs: BCS, swap_info_arg: vector<u8>): MoveCall {
        let type_arg = ascii::string(bcs.peel_vec_u8());
        transaction::new_move_call(
            transaction::new_function(
                package_id,
    ...
    public(package) fun sui_transfer_move_call(package_id: address, mut bcs: BCS, swap_info_arg: vector<u8>): MoveCall {
        let type_arg = ascii::string(bcs.peel_vec_u8());
        transaction::new_move_call(
            transaction::new_function(
                package_id,
