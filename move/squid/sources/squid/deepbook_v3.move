module squid::deepbook_v3 {
    ...
    public fun estimate<B, Q>(self: &mut SwapInfo, pool: &Pool<B, Q>, clock: &Clock) {
        ...
        assert!(&swap_data.base_type == &type_name::get<B>().into_string(), EWrongCoinType);
        assert!(&swap_data.quote_type == &type_name::get<Q>().into_string(), EWrongCoinType);
    ...
    public fun swap<B, Q>(self: &mut SwapInfo, pool: &mut Pool<B, Q>, squid: &mut Squid, clock: &Clock, ctx: &mut TxContext) {
        ...
        if (swap_data.has_base) {
            ...
            let mut base_in = self.coin_bag().balance<B>().destroy_some().into_coin(ctx);
            ...
            let deep_in = squid_coin_bag.exact_balance<DEEP>(deep_required).into_coin(ctx);
            ...
            let mut quote_in = self.coin_bag().balance<Q>().destroy_some().into_coin(ctx);
    ...
    public(package) fun swap_move_call(package_id: address, mut bcs: BCS, swap_info_arg: vector<u8>, squid_arg: vector<u8>): MoveCall {
        let mut pool_arg = vector[0];
        pool_arg.append(bcs.peel_address().to_bytes());
