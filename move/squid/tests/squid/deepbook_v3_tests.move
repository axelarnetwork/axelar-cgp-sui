...
module squid::deepbook_v3_tests {
    use deepbook::{pool::Pool, pool_tests};
    use squid::{deepbook_v3, swap_info, swap_type};
    use std::type_name;
    use sui::{
        clock::Clock,
    ...
    fun test_estimate() {
        ...
        squid.value_mut!(b"").coin_bag_mut().store_balance(deep_coin.into_balance());
        ...
        let deep_balance = squid.value_mut!(b"").coin_bag_mut().balance<DEEP>().destroy_some();
