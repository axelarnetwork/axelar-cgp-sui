...
fun test_estimate() {
    ...
    // When estimating, we should take 10bps worth of SUI out, then swap the rest for USDC.
    let clock = test.take_shared<Clock>();
    ...
    let deep_balance = squid.value_mut!(b"").coin_bag_mut().get_balance<DEEP>().destroy_some();
