module axelar_gateway::gateway_v1;
...
/// verification.
public struct Gateway_v1 has store {
    operator: address,
...
public(package) fun version_control(self: &Gateway_v1): &VersionControl {
...
#[syntax(index)]
fun borrow(self: &Gateway_v1, command_id: Bytes32): &MessageStatus {
    table::borrow(&self.messages, command_id)
}

#[syntax(index)]
fun borrow_mut(self: &mut Gateway_v1, command_id: Bytes32): &mut MessageStatus {
...
fun approve_message(self: &mut Gateway_v1, message: message::Message) {
    let command_id = message.command_id();
...
#[test_only]
public(package) fun signers_mut(self: &mut Gateway_v1): &mut AxelarSigners {
    &mut self.signers
}
...
#[test_only]
fun dummy(ctx: &mut TxContext): Gateway_v1 {
    new(
        @0x0,
        sui::table::new(ctx),
