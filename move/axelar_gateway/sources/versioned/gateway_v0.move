module axelar_gateway::gateway_v0 {
    use axelar_gateway::{
        ...
        channel::{Self, ApprovedMessage},
        events,
        message::{Self, Message},
        message_status::{Self, MessageStatus},
        message_ticket::MessageTicket,
        proof,
        weighted_signers
    };
    use std::ascii::String;
    use sui::{address, clock::Clock, hash, table::{Self, Table}};
    use utils::utils;
    ...
    public(package) fun approve_messages(self: &mut Gateway_v0, message_data: vector<u8>, proof_data: vector<u8>) {
    ...
    public(package) fun rotate_signers(
        ...
        let weighted_signers = utils::peel!(new_signers_data, |bcs| weighted_signers::peel(bcs));
        let proof = utils::peel!(proof_data, |bcs| proof::peel(bcs));
    ...
    public(package) fun is_message_executed(self: &Gateway_v0, source_chain: String, message_id: String): bool {
    ...
    public(package) fun take_approved_message(
        ...
        assert!(self[command_id] == message_status::approved(message.hash()), EMessageNotApproved);
    ...
    public(package) fun send_message(_self: &Gateway_v0, message: MessageTicket, current_version: u64) {
        let (source_id, destination_chain, destination_address, payload, version) = message.destroy();
    ...
    fun peel_messages(message_data: vector<u8>): vector<Message> {
        utils::peel!(message_data, |bcs| {
            let messages = vector::tabulate!(bcs.peel_vec_length(), |_| message::peel(bcs));
    ...
    #[test_only]
    public(package) fun messages_mut(self: &mut Gateway_v0): &mut Table<Bytes32, MessageStatus> {
        &mut self.messages
    }
    ...
    #[test_only]
    public(package) fun destroy_for_testing(self: Gateway_v0): (address, Table<Bytes32, MessageStatus>, AxelarSigners, VersionControl) {
        let Gateway_v0 {
            operator,
            messages,
            signers,
    ...
    #[test_only]
    public(package) fun approve_message_for_testing(self: &mut Gateway_v0, message: Message) {
    ...
    fun test_approve_message() {
        ...
        assert!(data.is_message_executed(
            source_chain,
            message_id,
        ) ==
        true, EMessageNotApproved);
