module axelar_gateway::proof {
    ...
    use sui::{bcs::BCS, ecdsa_k1 as ecdsa};
    ...
    // ---------
    #[test_only]
    public(package) fun create_for_testing(signers: WeightedSigners, signatures: vector<Signature>): Proof {
        Proof {
            signers,
            signatures,
        }
    ...
    fun test_validate() {
        let mut rng = sui::random::new_generator_for_testing();
        let mut keypairs = vector[0, 1, 2].map!(|_| ecdsa::secp256k1_keypair_from_seed(&rng.generate_bytes(32)));
    ...
    fun test_validate_redundant_signatures() {
        let mut rng = sui::random::new_generator_for_testing();
        let keypairs = vector[0, 1, 2].map!(|_| ecdsa::secp256k1_keypair_from_seed(&rng.generate_bytes(32)));
        let pub_keys = keypairs.map_ref!(|keypair| *keypair.public_key());
        let weights = vector[0, 1, 2].map!(|_| rng.generate_u64() as u128);
        let message = rng.generate_bytes(32);
        let nonce = axelar_gateway::bytes32::from_bytes(rng.generate_bytes(32));
