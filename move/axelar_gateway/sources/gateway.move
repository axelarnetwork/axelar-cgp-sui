...
module axelar_gateway::gateway {
    use axelar_gateway::{
        auth::{Self, validate_proof},
        bytes32::{Self, Bytes32},
        channel::{Channel, ApprovedMessage},
        gateway_v0::{Self, Gateway_v0},
        message_ticket::{Self, MessageTicket},
        owner_cap::{Self, OwnerCap},
        weighted_signers
    };
    use std::ascii::{Self, String};
    use sui::{clock::Clock, table, versioned::{Self, Versioned}};
    use utils::utils;
        ...
        let inner = versioned::create(
            ...
            gateway_v0::new(
                ...
                auth::setup(
                    ...
                    utils::peel!(initial_signers, |bcs| weighted_signers::peel(bcs)),
                    clock,
                    ctx,
                ),
                version_control(),
    ...
    /// This method is only intended to be called via a Transaction Block, keeping
    /// more flexibility for upgrades.
    entry fun rotate_signers(self: &mut Gateway, clock: &Clock, new_signers_data: vector<u8>, proof_data: vector<u8>, ctx: &TxContext) {
    ...
    public fun is_message_executed(self: &Gateway, source_chain: String, message_id: String): bool {
    ...
    fun test_setup() {
        ...
        let (operator_result, messages, signers, _) = inner.destroy<Gateway_v0>().destroy_for_testing();
    ...
    fun test_approve_messages() {
        ...
        let messages = vector<axelar_gateway::message::Message>[axelar_gateway::message::dummy()];
