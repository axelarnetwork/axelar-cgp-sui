...
async function registerInterchainToken(client, keypair, itsInfo, name, symbol, decimals, mintAmount = false) {
    ...
    if (mintAmount) {
        tx.moveCall({
            ...
            arguments: [tx.object(treasuryCap.objectId), tx.pure(mintAmount), tx.pure.address(keypair.getPublicKey().toSuiAddress())],
    ...
    const coinInfo = tx.moveCall({
        ...
        arguments: [tx.object(coinMetadata.objectId)],
        typeArguments: [coinType],
    });

    const coinManagement = tx.moveCall({
        target: `${itsPackageId}::coin_management::new_with_cap`,
        arguments: [tx.object(treasuryCap.objectId)],
        typeArguments: [coinType],
    });

    tx.moveCall({
        target: `${itsPackageId}::service::register_coin`,
        arguments: [tx.object(itsObjectId), coinInfo, coinManagement],
        typeArguments: [coinType],
    });
    ...
    const coinObjectId = mintAmount
        ? result.objectChanges.find((object) => object.objectType === `0x2::coin::Coin<${coinType}>`).objectId
        : null;
...
if (require.main === module) {
    ...
    (async () => {
        ...
        const tokenIdObj = tx.moveCall({
            ...
            arguments: [tx.pure.address(tokenId)],
            typeArguments: [],
        });
